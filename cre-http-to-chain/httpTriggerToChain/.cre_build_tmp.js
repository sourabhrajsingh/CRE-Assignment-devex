...
var configSchema = exports_external.object({
  priceFeeds: exports_external.record(exports_external.string(), exports_external.object({
    proxyAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/i),
    chainSelectorName: exports_external.string(),
    chainId: exports_external.number().int().positive()
  })),
  targetContract: exports_external.object({
    address: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/i),
    chainSelectorName: exports_external.string(),
    chainId: exports_external.number().int().positive(),
    gasLimit: exports_external.string()
  })
});
var priceFeedAbi = parseAbi([
...
var fetchAndParse = (sendRequester, apiKey, updatedAt, chainId) => {
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  if (!requestData.token)
    throw new Error("Error: 'token' field is required");
  const tokenUpper = requestData.token.toUpperCase();
  const feed = runtime2.config.priceFeeds[tokenUpper];
  if (!feed)
  ...
  runtime2.log(`Successfully read storage value: ${roundId}, answer: ${answer}`);
  const secret = runtime2.getSecret({ id: "CRE_ETHERSCAN_API_KEY" }).result();
  const apiKey = secret.value;
  const aggregatedBlockNumber = httpClient.sendRequest(runtime2, fetchAndParse, consensusMedianAggregation())(apiKey, updatedAt, feed.chainId).result();
  runtime2.log(`Aggregated Etherscan block number result: ${aggregatedBlockNumber}`);
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.targetContract.address,
    report: reportResponse,
    gasConfig: { gasLimit: runtime2.config.targetContract.gasLimit }
  }).result();
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    const txHash = bytesToHex3(writeResult.txHash || new Uint8Array(32));
    runtime2.log(`Snapshot written on-chain! Tx: ${txHash}`);
    return txHash;
  }
  throw new Error(`Write failed: ${writeResult.txStatus} - ${writeResult.errorMessage || "unknown"}`);
};
